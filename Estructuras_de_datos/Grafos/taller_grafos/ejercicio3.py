'''
Dado un grafo dirigido escriba una función que identifique si hay ciclos en él.
 Un ciclo es una secuencia de vértices conectados por aristas, 
 donde el vértice inicial y final son el mismo.
'''
...
class Graph:
  ...
  def has_cycle_helper(self,star_vertex, visited):
    custom_queue = queue()
    custom_queue.enqueue((star_vertex, None))

    while not custom_queue.is_empty():
      
      current_vertex, parent = custom_queue.dequeue()

      if current_vertex in visited and current_vertex == star_vertex:
        return True

      visited.add(current_vertex)

      for adjacenct_vertex in self.adjacency_list:
        if adjacenct_vertex != parent:
          custom_queue.enqueue((adjacenct_vertex, current_vertex))
